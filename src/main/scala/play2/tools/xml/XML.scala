...
trait XML {
  def apply[T](t: T, base: xml.NodeSeq = xml.NodeSeq.Empty)(implicit w: XMLWriter[T]): xml.NodeSeq = w.write(t, base)
...
trait BasicReaders {
  ...
  implicit object BooleanReader extends XMLReader[Boolean] {
    def read(x: xml.NodeSeq): Option[Boolean] =
      if (x.isEmpty) None
      else Try(x.text.toBoolean).toOption
  }
