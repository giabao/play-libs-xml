...
class SOAPSpec extends Specification {
  ...
  "SOAP" should {
    ...
    "deserialize SOAP" in {
      SOAP.fromSOAP[Foo](
        testSoapMessage
      ) must equalTo(Some(testObject)).ignoreSpace
    }
    ...
    "return None if faultcode parameter is missing" in {
      val soapMessage =
        <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:typ="http://interfacedepot.acsia.gip_info_retraite.fr/types/">
          <soapenv:Body>
            <soapenv:Fault>
